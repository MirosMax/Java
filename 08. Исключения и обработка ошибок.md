...
- [[#Precise rethrow|Precise rethrow]]
...
При перехвате и повторном выбрасывании компилятор сохраняет **точный тип** исключения, если параметр `catch` — `final` или effectively final:

```java
void process() throws IOException, SQLException {
    try {
        risky();
    } catch (Exception e) {           // параметр — Exception
        log(e);
        throw e;                       // throws IOException, SQLException (точный тип)
    }
}
```

Без precise rethrow пришлось бы объявлять `throws Exception`. Механизм работает только для checked исключений, не расширяет `throws` unchecked.
