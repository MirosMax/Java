...
- [[#Scoped Values (Java 21)|Scoped Values (Java 21)]]
...
Изоляция состояния по потокам: у каждого потока своя копия значения — без синхронизации и гонок (подробно — [[15. JVM, память и профессиональная разработка#ThreadLocal|ThreadLocal]]):

```java
ThreadLocal<String> user = ThreadLocal.withInitial(() -> "аноним");

// в потоке-обработчике запроса
user.set("Аня");
String u = user.get();
user.remove();   // важно в пулах потоков — иначе утечка
```

Типовое применение: контекст запроса, идентификатор транзакции, «текущий» объект в цепочке вызовов.
...
Для рекурсивных параллельных вычислений «разделяй и властвуй» существует отдельный фреймворк — **Fork/Join Framework** (`ForkJoinPool`, `RecursiveTask`), см. [[17. Конкурентные утилиты#Fork/Join Framework|Fork/Join]].
...
### `ShutdownOnSuccess` и `ShutdownOnFailure`

```java
// завершить, когда первая задача успешна
try (var scope = new StructuredTaskScope.ShutdownOnSuccess<String>()) {
    scope.fork(() -> fetchFromPrimary());
    scope.fork(() -> fetchFromSecondary());
    String result = scope.join().result();   // первый успешный
}
```

## Scoped Values (Java 21)

Безопасная альтернатива `ThreadLocal` для передачи неизменяемых контекстных данных. В отличие от `ThreadLocal`, не требует `remove()` и не даёт утечек памяти:

```java
private static final ScopedValue<String> USER = ScopedValue.newInstance();

void handleRequest() {
    ScopedValue.where(USER, "Аня", () -> {
        // внутри this Runnable USER доступен
        process();           // USER.get() → "Аня"
    });
    // после выхода USER не определён
}

void process() {
    String user = USER.get();   // "Аня"
    // USER не может быть изменён
}
```

Scoped Values неизменяемы — значение устанавливается один раз для области видимости. Поддерживаются виртуальными потоками. Предпочтительнее `ThreadLocal` для всех новых проектов.
